...
function openArticle(article) {
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-0">
                <div class="lg:col-span-3 p-6 md:p-16 border-r border-black">
                    ...
                    <div class="flex items-center gap-6 mb-12 border-b border-black/10 pb-6">
                        <span class="text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><i data-lucide="share-2" class="w-4 h-4"></i> Deel artikel</span>
                    ...
                    <div class="prose max-w-none text-lg leading-relaxed font-medium space-y-6 text-gray-800">${article.content || "<p>Gedetailleerde feitelijke informatie zodat ik niet voor verrassingen kom te staan.</p>"}</div>
                ...
                <div class="lg:col-span-1 bg-gray-50 p-6 md:p-8">
                    <div class="sticky top-10 space-y-8">
                        <div class="bg-white border-2 border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center text-center">
                            ...
                            <div class="w-20 h-20 bg-black text-white flex items-center justify-center mb-6 rounded-full"><i data-lucide="bar-chart-3" class="w-10 h-10"></i></div>
    ...
    document.querySelectorAll('.related-item').forEach(item => {
        item.onclick = () => {
            ...
            if(selected) { openArticle(selected); window.scrollTo(0,0); }
        };
    });
    if (window.lucide) lucide.createIcons();
}
...
function renderHome() {
        ...
        data.videos.slice(0, 2).forEach(v => {
            ...
            vEl.innerHTML = `
                <div class="relative w-full aspect-video mb-3 bg-gray-200 border border-black overflow-hidden shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                    ...
                    <div class="absolute inset-0 bg-black/20 group-hover:bg-transparent flex items-center justify-center transition-all"><i data-lucide="play-circle" class="text-white w-10 h-10"></i></div>
                </div>
    ...
    if (aContainer) {
        ...
        data.analysis.slice(0, 3).forEach(a => {
            ...
            aEl.innerHTML = `<div class="text-xs font-bold uppercase mb-2 text-gray-400 group-hover:text-black transition-colors">${a.date}</div><h3 class="font-extrabold uppercase text-xl mb-3 leading-tight">${a.title}</h3><p class="text-sm font-semibold line-clamp-2 text-gray-600">${a.intro}</p>`;
    ...
    if (homeAd) {
        homeAd.innerHTML = `<div class="flex flex-col items-center p-6 bg-gray-50 border border-black/10 h-full"><span class="text-[9px] text-gray-400 mb-6 tracking-[0.3em] font-bold uppercase">Advertentie</span><div class="w-full aspect-square bg-white border border-black mb-6 overflow-hidden shadow-[4px_4px_0_0_rgba(0,0,0,0.1)]"><img src="https://images.unsplash.com/photo-1551288049-bbbda5366392?q=80&w=400" class="grayscale w-full h-full object-cover"></div><h4 class="text-xs font-black uppercase mb-3 text-center">BI Masterclass 2026</h4><button class="border-2 border-black px-4 py-2 text-[10px] font-bold uppercase hover:bg-black hover:text-white transition-all w-full">Informatie aanvragen</button></div>`;
    }
    ...
    if (rContainer) {
        ...
        data.rawData.forEach(r => {
            ...
            rEl.className = 'border-l-4 border-black pl-3 text-sm font-semibold py-2 mb-4 hover:bg-gray-50 transition-colors';
            rEl.textContent = r;
...
function renderTopics() {
    ...
    data.analysis.forEach(a => {
        ...
        el.innerHTML = `<div class="aspect-video w-full overflow-hidden border-b border-black"><img src="${a.image || 'placeholder.jpg'}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500"></div><div class="p-6 flex flex-col flex-grow"><h3 class="font-extrabold uppercase text-xl mb-3 leading-tight">${a.title}</h3><p class="text-sm font-semibold mb-6 line-clamp-3 text-gray-600">${a.intro}</p><div class="text-[10px] font-bold uppercase border-t border-black pt-3 mt-auto">${a.date}</div></div>`;
...
function renderPortfolio() {
    ...
    data.portfolio.forEach(item => {
        ...
        el.innerHTML = `<div class="aspect-video w-full overflow-hidden border-b border-black"><img src="${item.image || 'placeholder.jpg'}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500"></div><div class="p-6"><div class="text-[10px] font-bold uppercase text-gray-400 mb-2">${item.date}</div><h3 class="text-xl font-extrabold uppercase mb-4 leading-tight">${item.title}</h3><p class="text-sm font-semibold text-gray-700 line-clamp-2">${item.intro}</p></div>`;
