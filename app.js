...
function renderHome() {
    ...
    if (fContainer && data.featured) {
        fContainer.innerHTML = `
            <div class="md:col-span-2 border-r border-black relative border-b md:border-b-0 overflow-hidden group">
                <img src="${data.featured.image}" alt="Featured" class="w-full h-full object-cover min-h-[400px] grayscale contrast-125 group-hover:grayscale-0 transition-all duration-700">
            </div>
        ...
        data.videos.forEach(v => {
            ...
            vEl.innerHTML = `
                <div class="relative w-full h-40 mb-3 bg-gray-200 border border-black overflow-hidden">
                    <img src="${v.thumb}" class="w-full h-full object-cover grayscale contrast-125 group-hover:grayscale-0 transition-all">
...
function renderTopics() {
    ...
    function renderTopicGrid(filter) {
        ...
        items.forEach(a => {
            ...
            el.className = 'border-r border-b border-black flex flex-col cursor-pointer hover:bg-gray-50 group';
            el.innerHTML = `
                <div class="aspect-video w-full overflow-hidden border-b border-black bg-gray-100">
                    <img src="${a.image || 'placeholder.jpg'}" class="w-full h-full object-cover grayscale contrast-125 group-hover:grayscale-0 transition-all duration-500">
                </div>
                <div class="p-6 flex flex-col justify-between flex-grow">
                    ...
                    <div class="text-xs font-bold uppercase border-t border-black pt-2 text-gray-400">${a.date}</div>
                </div>
            `;
...
function renderportfolio() {
    ...
    data.portfolio.forEach(item => {
        ...
        el.innerHTML = `
            <div class="aspect-video w-full overflow-hidden border-b border-black bg-gray-100">
                <img src="${item.image || 'placeholder.jpg'}" class="w-full h-full object-cover grayscale contrast-125 group-hover:grayscale-0 transition-all duration-500">
            </div>
